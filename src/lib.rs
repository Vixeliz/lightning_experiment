...
fn main() {
    App::new()
        ...
        .add_systems((lightning, add_lightning.after(draw), draw))
...
fn add_lightning(
    ...
    touches: Res<Touches>,
    mut last_tap: Local<(Duration, u64)>,
) {
    ...
    if let Ok((camera, global_transform)) = camera_q.get_single() {
        if let Some(world_position) = touches.iter().next().and_then(|cursor| {
            ...
            camera.viewport_to_world_2d(global_transform, cursor)
        }) {
            if let Ok((mut lightning, entity)) = temp_lightning.get_single_mut() {
                if time.elapsed() - last_tap.0 < Duration::from_millis(500)
                    && touches.iter().next().unwrap().id() != last_tap.1
                {
            ...
            last_tap.0 = time.elapsed();
            last_tap.1 = touches.iter().next().unwrap().id();
        }
